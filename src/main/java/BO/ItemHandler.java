...
public class ItemHandler {
    ...
    public static ItemInfo getById(String searchTerm) throws SQLException {
        Item item = Item.getById(searchTerm);
        return new ItemInfo(item.getId(),item.getType(), item.getName(), item.getStock(), item.getPrice());
    }

    public static Collection<ItemInfo> getByType(String searchTerm) throws SQLException {
    ...
    public static Collection<ItemInfo> getAll() throws SQLException {
        ...
        return items.stream().map(item -> new ItemInfo(item.getId(),
                                                        item.getType(),
                                                        item.getName(),
                                                        item.getStock(),
                                                        item.getPrice()))
